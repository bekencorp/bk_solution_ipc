...
#if CONFIG_BK_H264E_DEBUG_SEI
        /* Debug: append a transport-diagnostic SEI NAL to the H.264 frame right
         * before sending, on this transfer thread (decoupled from the encoder).
         * Receiver uses it to detect frame loss/reorder/truncation/corruption. */
        if (frame->fmt == PIXEL_FMT_H264 && frame->length > 0U)
        {
            uint32_t new_len = bk_h264_debug_sei_append(frame->frame,
                                                        frame->length,
                                                        frame->size,
                                                        frame->sequence);
            if (new_len > frame->length)
            {
                frame->length = new_len;
#if CONFIG_L2_CACHE_ENABLE || CONFIG_DCACHE
                flush_dcache(frame->frame, (long)new_len);
#endif
            }
        }
#endif
