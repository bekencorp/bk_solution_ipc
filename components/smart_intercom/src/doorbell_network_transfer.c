...
#endif
    /* Control channel is JSON-framed (CONFIG_NTWK_CTRL_CHAN_JSON): the payload
     * is a complete, reassembled JSON-RPC 2.0 text. */
    doorbell_jsonrpc_handle_cmd((const char *)data, length);
    return BK_OK;
}

bk_err_t doorbell_bk_net_video_recv(uint8_t *data, uint32_t length)
{
    /* Downlink (app -> device) H.264 access units for local display. */
    return doorbell_downlink_video_recv(data, length);
}
