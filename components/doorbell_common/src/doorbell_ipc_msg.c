...
#define DOORBELL_IPC_TAG "DOORBELL_IPC"

#define LOGI(...)       BK_LOGI(DOORBELL_IPC_TAG, ##__VA_ARGS__)
#define LOGW(...)       BK_LOGW(DOORBELL_IPC_TAG, ##__VA_ARGS__)
#define LOGE(...)       BK_LOGE(DOORBELL_IPC_TAG, ##__VA_ARGS__)
#define LOGD(...)       BK_LOGD(DOORBELL_IPC_TAG, ##__VA_ARGS__)
#define LOGV(...)       BK_LOGV(DOORBELL_IPC_TAG, ##__VA_ARGS__)
...
{
    int ret = BK_OK;
    doorbell_ipc_keepalive_cfg_t cfg = {0};
    ...
    ret = bk_wdrv_customer_transfer(DOORBELL_IPC_CMD_KEEPALIVESTART, (uint8_t *)&cfg, sizeof(cfg));
...
bk_err_t doorbell_ipc_stop_keepalive(void)
{
    ...
    os_memset(&cfg, 0, sizeof(doorbell_ipc_keepalive_cfg_t));

    ret = bk_wdrv_customer_transfer(DOORBELL_IPC_CMD_KEEPALIVESTOP, (uint8_t *)&cfg, sizeof(cfg));
    if (ret != 0) {
        LOGE("%s: Failed to send DOORBELL_IPC_CMD_KEEPALIVESTOP: %d\n", __func__, ret);
        return BK_FAIL;
    } 
...
int doorbell_ipc_wakeup_env_init(void)
{
    ...
    ret = bk_wdrv_customer_transfer_rsp(DOORBELL_IPC_CMD_GET_WAKEUP_ENV_ADDR, NULL, 0, 
    ...
    if (response_len >= sizeof(uint32_t)) {
        doorbell_ipc_wakeup_env_response(response_buf, response_len);
...
void doorbell_ipc_wakeup_env_response(uint8_t *data, uint16_t len)
{
