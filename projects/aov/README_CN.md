...
- CP 持有产品主状态、AON RTC 1 秒检测定时器和 AP 上下电投票；
- AP 按 CP 预置 job 执行二维码配网、移动检测或工作图传；
- CP 管理 `240×240 Y8` 上一帧灰度缓冲；
- AP 必须回报 `LAST_GRAY_READY` 和 `POWERDOWN_READY` 后，CP 才允许 AP 下电；
- 已有网络配置时 AP 不进入二维码配网：AP 联网成功后直接从 ISP SP 通道抓取 240×240 NV12 的 Y 平面，并调用 SDK `motion_detect_compare_gray()` 进入移动检测；
- 真实二维码识别、事件录像和人形检测通过 backend ops 后续接入。
...
`aov_sm inject` 仅用于状态机 smoke 测试，不能替代真实 WiFi、Camera、AI 或 Recorder 验证。

`aov_wifi` 是调试命令：配置写入 CP EasyFlash 的 `aov_device_info`，`show` 不显示密码。重启后 CP 检测到有效配置会启动 AP，并通过跨核 IPC 下发凭据；WiFi STA 连接必须由 AP 发起，AP 获取 IP 或连接失败后再回报 CP 推进状态机。当前密码为明文调试存储，量产版本必须加密。

状态机详细设计见工作区 `docs/ipc_device_state_machine_design.md`。
