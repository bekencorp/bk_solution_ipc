...
> Interfaces, state transitions, low-power behavior, and test coverage are still evolving.
> This project is not ready for production release.
...
## 6 AOV CP/AP State-Machine Framework

The first framework slice provides:

- CP-owned product state, one-second AON RTC detection timer, and AP power voting;
- AP job execution for QR provisioning, motion-check, and live-stream profiles;
- a CP-owned `240x240 Y8` previous-gray buffer;
- AP shutdown gating on `LAST_GRAY_READY` and `POWERDOWN_READY`;
- when credentials exist, AP skips QR provisioning, connects WiFi, captures the
  240x240 NV12 Y plane from ISP SP, and runs the SDK gray-frame motion detector;
- backend ops for later QR, recorder, and human-detection integration.
...
aov_sm status
aov_sm gray_info
aov_sm inject <event_id> [result]
aov_wifi set <ssid> <password|->
aov_wifi show
aov_wifi clear
```

The injection command is a state-machine smoke-test aid; it is not a substitute
for real WiFi, camera, AI, or recorder validation.

`aov_wifi` is debug-only. It stores credentials in the CP EasyFlash
`aov_device_info` record and never prints the password. On reboot, CP boots AP
and sends the credentials over inter-core IPC. AP must initiate the WiFi STA
connection and report success or failure to CP. The current debug record stores
the password in plaintext; production firmware must encrypt it.

See `docs/ipc_device_state_machine_design.md` in the workspace for the design.
