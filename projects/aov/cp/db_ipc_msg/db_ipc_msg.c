...
int db_ipc_msg_handler(struct bk_msg_hdr *msg)
{
    ...
    switch (db_ipc_cmd_hdr->cmd_id)
    {
        ...
        case DB_IPC_CMD_AOV_GET_SHARED_ENV_ADDR:
        {
            static uint32_t addr;
            aov_shared_env_t *shared_env = aov_cp_state_machine_get_shared_env();
            addr = (uint32_t)(uintptr_t)shared_env;
            cfm_data = (uint8_t *)&addr;
            cfm_data_len = sizeof(addr);
            LOGI("DB_IPC_CMD_AOV_GET_SHARED_ENV_ADDR: 0x%x\n", addr);
            break;
        }
        case DB_IPC_CMD_AOV_AP_REPORT:
        {
            if (db_ipc_cmd_hdr->len != sizeof(aov_ap_report_t))
            {
                LOGE("invalid AP report len=%u expected=%u\n",
                     db_ipc_cmd_hdr->len, (unsigned)sizeof(aov_ap_report_t));
                ret = BK_ERR_PARAM;
                break;
            }

            aov_ap_report_t report;
            os_memcpy(&report, db_ipc_cmd_hdr + 1, sizeof(report));
            ret = aov_cp_state_machine_on_ap_report(&report);
            if (ret != BK_OK)
            {
                LOGE("queue AP report failed: %d\n", ret);
            }
            break;
        }
        default:
